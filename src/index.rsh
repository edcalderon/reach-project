...
export const main = Reach.App(() => {
  ...
  while (true) {
    ...
    Bidder.publish(bid, myAddress).when(bid > price && owner != this && this !== lastBidder).timeout(relativeTime(deadline), () => {
      each([Creator, Bidder], () => {
        interact.seeOutcome(nftViewAddress, owner);
      });
      Bidder.pay(price).when(lastBidder == this).timeout(relativeTime(20), () => {
        each([Creator, Bidder], () => {
